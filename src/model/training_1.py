...
def train_and_evaluate(train_model_spec, eval_model_spec,
    ...
    with tf.Session() as sess:
        ...
        if restore_from is not None:
            ...
            [best_eval_metric] = load_best_metric(best_json_path)
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            num_steps = 5#(params.train_size + params.batch_size - 1) // params.batch_size
            train_sess(sess, train_model_spec, num_steps, train_writer, params)
            # Save weights
            last_save_path = os.path.join(model_dir, 'last_weights', 'after-epoch')
            logging.info('last_save_path: ', str(last_save_path))
